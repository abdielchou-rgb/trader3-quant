...
trader3/            # 核心包（10 个 Tool，全部经 IronGate）
  ...
  research/         # 研究层（R1-R9）：experiment/labeling/rigor/sample_weights/
                    #   factor_library(Alpha158)/collaborative/ml_pipeline/meta_ml/
                    #   stock_selector(滚动ML选股组合)
...
docs/quant-knowledge/  # 全景认知库：qlib/mlfinlab/框架对比/因子论文 解剖笔记 + 30 天路线图
config/             # 策略/约束 YAML
scripts/            # update_market_data.py（qlib_bin 增量管线）等
shared_state/       # 原子写共享状态 + paper/（纸面账户）+ _quarantine_pre_audit/
tests/              # 656 个测试（654 passed / 2 skipped 基线）；testpaths 已在 pyproject 隔离
```
...
- **Meta-label 分类选股**（trader3/research/meta_ml.py，R2 labeling 激活）：
  标签 y=1 ⟺ 次日扣成本收益>0（该不该开仓），GradientBoostingClassifier → p(win)；
  真实 csi300 冒烟 top10% 命中率 0.388 > 基础 0.341（+4.7pp），top30/50% 单调
  优于基础 —— meta 模型在弱市仍筛出更高命中率子集
- **滚动 ML 选股组合**（trader3/research/stock_selector.py，R9）：walk-forward
  滚动重训（[r-lookback,r] 训练 → [r,r+rebal) 预测，无前视），回归选 top-N +
  可选 meta 过滤 → 等权组合 → 日频净值/命中率；合成可学数据组合 > 全池等权；
  真实 csi300 弱市诚实结果（年化≈-4.9%，无 alpha 市场不虚报超额）
...
3. **量化输出是候选信号，非投资建议**
4. **测试必须绿** — 当前基线 654 passed / 2 skipped
