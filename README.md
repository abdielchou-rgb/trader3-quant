...
> **⚠ 状态更新（2026-08-24）**：本项目已完成三轮深度审计修复（117 tests 全绿）。
> 回测引擎已消除同日前视/幸存者偏差并建模涨跌停约束；估值启用白名单模式；
> 信号引擎伪造指标已清除；v2 触发→风控→纸面执行闭环已通电；qlib_bin 增量管线已上线
> （\python scripts/update_market_data.py --apply\）。
> 行为契约与数据纪律详见 [CLAUDE.md](CLAUDE.md)。下文为历史文档，部分 M 阶段描述已过时。
